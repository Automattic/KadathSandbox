...
11. **Settled means agreed, not merely quiet** (from the second 38-case Offering pass: 47 of 49 cases queued for the Deep Scrying because `decided_by != agree` fired whenever the deterministic layer was lower than a red the Cavern and the model both gave). A case is settled when the Cavern and the model agree, the deterministic layer does not exceed them, and confidence ≥ 0.6; an agreed red is settled even if the run errored (a Deep Scrying cannot add runtime evidence the run did not produce); a green settles only with `coverage: full`; amber never settles. Also from that pass: adopted samples are loaded on `plugins_loaded` (pluggable functions such as `is_user_logged_in` are not defined while plugin files are being included); `PHP Parse error` counts as a fatal; the run's own fatal lines are recorded as `run.fatals` and shown to the Offering and the Deep Scrying; a model claim of `errored` without an engine-recorded fatal is downgraded to `full`; the engine's stderr is kept beside the case; adopted plugins are activated by editing `active_plugins` rather than through the activation sandbox; the offer breaker counts only unhealthy-stack failures.
