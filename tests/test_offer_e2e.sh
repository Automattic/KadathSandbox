#!/usr/bin/env bash
# End-to-end acceptance for bin/kadath offer. Requires the stack (make up) and
...
RUN=$(python3 bin/kadath offer tests/probe.php --json)
...
if [ -f "$BD" ]; then
  RUN2=$(python3 bin/kadath offer "$BD" --json)
  DIR2=$(dirname "$RUN2")
  ...
  ck "backdoor: iocs.json validates vs schema"    'python3 tests/validate_iocs.py "'"$DIR2"'/iocs.json" .claude/skills/kadath-scry/references/iocs-schema.json'
